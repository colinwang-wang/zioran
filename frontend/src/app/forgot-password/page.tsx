...
export default function ForgotPasswordPage() {
  ...
  const handleSendEmail = async () => {
    ...
    } catch { setError('发送邮箱验证码失败'); loadCaptcha(); }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !emailCode || !newPassword) { setError('请填写所有字段'); return; }
    setLoading(true); setError('');
    try {
      await forgotPassword({ email, email_code: emailCode, new_password: newPassword });
      setSuccess(true);
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      <div className="w-full max-w-sm bg-canvas rounded-lg p-8 border border-hairline shadow-sm">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-mute mt-1">通过邮箱验证码重置密码</p>
        </div>
        {success ? (
